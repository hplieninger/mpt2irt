...
#' beta <- cbind(runif(J,0,1),
#'               runif(J,0,1),
#'               rnorm(J,0,2))
#'               
#' # analyze and plot
#' gen <- boeck.gen.2012(N=N, J=J, beta=beta, traitItem=traitItem, prop.rev=.5)
#' fit <- boeck.fit(gen$X, gen$revItem, gen$traitItem, fitModel="2012",
...
#'                true.par=list(theta=gen$theta, beta=gen$beta))
#'                
#' # check with stan
#' fit2 <- boeck.fit(gen$X, gen$revItem, gen$traitItem, fitModel="2012",
...
#' @param fit a fitted object from \link{boeck.fit} (either Boeckenholt 2012 or extended)
#' @param S number of response styles in model (2012: 2; extended: 3)
#' @param N number of participants
#' @param J number of items
...
#' @import ggplot2
#' @export
plot_RS <- function(fit, S, N, J, revItem=rep(0,J), traitItem=rep(1,J), trait="sample"){
  rs.names <- c("MRS","ERS", "ARS")
...
#   J <- length(grep("beta", colnames(fit[[1]])))/(S+1)
  ...
  for(s in 1:S){
    beta.rs <- combine.mcmc(fit[, paste0("beta[",1:J,",",s,"]")])
    M <- nrow(beta.rs)
    p.item.rs <- matrix(NA, M, J)
    
    # sample: f
    if(trait == "sample"){
      theta.rs <- colMeans(combine.mcmc(fit[, paste0("theta[",1:N,",",s,"]")]))
      p.item.rs <- apply(beta.rs, 2, function(x) colMeans(pnorm(outer(theta.rs, x,"-"))))
      summary(p.item.rs)
    # average: 
    }else{
      p.item.rs <- apply(beta.rs, 2, function(x) pnorm(as.numeric(trait)-x))
    }
    
    ss[seq(s,S*J, S), 3:7] <- t(apply(p.item.rs, 2, function(x) c( mean(x), sd(x),quantile(x, c(.025, .5, .975)))))
    ss[seq(s,S*J, S),1:2] <-cbind( 1:J,  rs.names[s])
  }
  
  ss$Type <- factor(ss$Type, levels=rs.names)
  ...
  gg <- ggplot(aes(x=Item, y=Mean, col=revItem), data=ss)+geom_point()+theme_bw()+
    facet_wrap(Type~traitItem,nrow = S,scales = "free_x")+
    geom_errorbar(aes(ymin=q025, ymax=q975))+ylim(0,1)+
    ggtitle(paste0("Proportion of Responses due to Response Styles (Mean, 95% CI, trait=",trait,")"))
  plot(gg)
...
plot_GRS <- function(fit, parameter = "beta", estimate = "point", plot = TRUE,
  ...
  grs <- gelman.diag(fit, multivariate = F)
  if (plot == TRUE) {
...
plot_Geweke <- function(fit, parameter = "beta", D = 1, frac1=0.1, frac2=0.5,
  ...
  gew <- geweke.diag(fit, frac1 = frac1, frac2 = frac2)
  if (plot == TRUE) {
