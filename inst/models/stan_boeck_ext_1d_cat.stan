...
Sigma <- diag_matrix(xi_theta) * Sigma_raw* diag_matrix(xi_theta);
 ...
 for(i in 1:N){	
	...
	for(j in 1:J){
		...
		acquies[i,j] <-  Phi(theta[i,4]-beta[j,4]);
		...
		p_cat[i,j,4] <- (1-acquies[i,j])*(1-middle[i,j])*trait[i,j]*(1-extreme[i,j]) + acquies[i,j]*(1-extreme[i,j]);
		p_cat[i,j,5] <- (1-acquies[i,j])*(1-middle[i,j])*trait[i,j]*extreme[i,j]     + acquies[i,j]*   extreme[i,j];
  }
}
